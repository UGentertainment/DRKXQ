(function() {
    ...
    if (window.Decrypter && Decrypter.extToEncryptExt) {
        ...
        Decrypter.extToEncryptExt = function(url) {
            ...
            return originalEncryptedExtension.call(this, cleanUrl) + '?v=20260908-1';
        };
    }
    ...
    ImageManager.loadBitmap = function(folder, filename, hue, smooth) {
        ...
        folder = manifest.imageFolders && manifest.imageFolders[folderKey] || folder;
    ...
    if (window.AudioManager && AudioManager.createBuffer) {
        ...
        AudioManager.createBuffer = function(folder, name) {
            var folderKey = String(folder || '').replace(/\\/g, '/').toLowerCase();
            var directory = manifest.audio[folderKey];
            folder = manifest.audioFolders && manifest.audioFolders[folderKey] || folder;
